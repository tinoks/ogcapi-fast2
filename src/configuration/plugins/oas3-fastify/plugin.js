...
module.exports = function (instance, opts, next) {
  ...
  // MADE BY TINO - associate schema ids 
  opts.spec.$id = opts.id ?? "oas3-spec";
  opts.spec = JSON.parse(JSON.stringify(opts.spec).replace(/#\//g,opts.spec.$id+'#/'));
  ...
  for (const path in opts.spec.paths) {
    ...
    for (const method in pathSpec) {
      ...
      for (const status in oasRoute.responses) {
        ...
        let responseSchema = responseSpec?.content?.['application/json']?.schema
        //if(Object.keys(responseSchema) == "$ref"){}
        //responseSchema.$ref =  responseSchema.$ref.replace("#", "$#");
        // add to route
